...
<tr>
	<th>Booking Id</th><th>UserName</th><th>Movie</th><th>Theater</th><th>Show Time</th><th>Date Of Show</th><th>Number Of Seats To Be Booked</th><th>Payment Amount</th>
</tr>
<%
try{
	/* DataBase Connection */
	...
	if (con != null) {
		...
		int bid=0,mid=0,sid=0,NoOfSeats=0,payment_amt=0;
		String username="";
		while(rs.next()){
			bid=rs.getInt(1);
	...
	%>	
		...
		<tr>
			<td><%=bid%></td><td><%=username%></td><td><%=m_name%></td><td><%=t_name%></td><td><%=show_time%></td><td><%=date%></td><td><%=NoOfSeats%></td><td><b><%=payment_amt%>rs</b></td>
		</tr>
<%}
}		
}catch(Exception e){
